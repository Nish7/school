...
public class AddOddNumbers {
  public static void main(String[] args) {
    ...
    while (ab.hasNextInt()) {
      ...
      // Use a while loop, check for another integer num using Scanner in (hint:
      // in.hasNextInt())
      // Get the next int and assign to variable. Use an if statement to check if the
      // number is between a and b and is odd.
      // If so, add to the sum variable
      while (in.hasNextInt()) {
        int n = in.nextInt();

        if ((n > a && n < b && n % 2 != 0) || (n > b && n < a && n % 2 != 0)) {
          sum += n;
        }
      }
