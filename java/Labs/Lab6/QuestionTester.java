...
class Question {
   ...
   /**
    * A constructor method to initialize text to parameter questionText, set answer
    * to empty String
    * 
...
class FillInQuestion extends Question {
   ...
   public FillInQuestion(String questionText) {
      ...
      int beginIndex = questionText.indexOf(underscore);

      // 2. second underscore, hint indexOf(char, from)
      int endIndex = questionText.indexOf(underscore, beginIndex + 1);

      // 3. extract the answer string Hint: use substring
      String sub = questionText.substring(beginIndex + 1, endIndex);
      ...
      String text = questionText.replaceAll(sub, "_".repeat(sub.length()));
