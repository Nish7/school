...
public class RemoveBadPairs {
	...
	static ArrayList<Integer> removeBadPairs(ArrayList<Integer> pairs) {
		...
		// 2. Check to see if the given array list pairs has an odd number of integer
		// elements
		...
		// Hint: increment your loop index by 2 and make sure your loop index is <
		// size()-1
		for (int i = 0; i < pairs.size() - 1; i += 2) {
			int l_ele = pairs.get(i);
			int r_ele = pairs.get(i + 1);
