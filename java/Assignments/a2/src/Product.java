...
public class Product {
	...
	public void rate(int stars) {
		this.ratings.put(stars, this.ratings.getOrDefault(stars, 0) + 1);
	}
	...
	public Double getAverageRating() {
		...
		if (this.ratings.keySet().size() == 0)
			return sum;

		for (Integer stars : this.ratings.keySet()) {
			sum += this.ratings.get(stars) * stars;
			cnt += this.ratings.get(stars);
		}
