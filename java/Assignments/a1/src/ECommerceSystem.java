...
public class ECommerceSystem {
  ...
  public ECommerceSystem() {
    ...
    products.add(new Book("Book", generateProductId(), 45.0, 4, 2, "Ahm Gonna Make You Learn", "T. McInerney", 2019));
    ...
    products.add(new Book("Book", generateProductId(), 35.0, 4, 2, "How to Fool Your Prof", "D. Umbast", 2000));
    products.add(new Book("Book", generateProductId(), 45.0, 4, 2, "How to Escape from Prison", "A. Fugitive", 2018));
    products.add(
    ...
    products.add(
        new Shoe("Air Force 2", generateProductId(), 180.0, new int[] { 10, 11, 12, 13, 1 },
            new int[] { 12, 13, 41, 1, 1 }));
    products
        .add(new Shoe("Air Jordan", generateProductId(), 120.0, new int[] { 11, 0, 2, 3, 1 },
            new int[] { 1, 1, 11, 30, 10 }));
  ...
  /*
   * Given a customer id, print all the current orders and shipped orders
   * for them
   * (if any)
   */
  // Print Order History
  public boolean printOrderHistory(String customerId) {
    ...
    if (!fnd) {
      this.errMsg = "Customer Id not found";
      return false;
    }
    ...
    for (Product product : products) {
      if (product.getId().equals(productId)) {
        ...
        if (!product.validOptions(productOptions)) {
          this.errMsg = "Product " + product.getClass().getName() + " ProductId " + productId + " Invalid Options:"
              + productOptions;
          return null;
        }

        if (product.getStockCount(productOptions) < 1) {
          this.errMsg = "Product " + productId + "is out of stock";
          return null;
        }

        String order_number = generateOrderNumber();
        orders.add(new ProductOrder(order_number, product, customers.get(cust_idx), productOptions));
  ...
  public void sortByPrice() {
    Collections.sort(products, new ProductPrice());
  }

  // Sort products alphabetically by product name
  public void sortByName() {
    Collections.sort(products, new ProductName());
  }
  ...
  public boolean booksByAuthor(String authorName) {
    ArrayList<Book> authorBook = new ArrayList<Book>();
    boolean e = false;

    // Collect all booksByAuthor and check if it exists
    for (Product p : products) {
    ...
    // Set ErrMsg
    if (!e) {
      this.errMsg = "No Author " + authorName + " exists";
      return false;
    }

    // Sort
    Collections.sort(authorBook, new sortAuthor());
